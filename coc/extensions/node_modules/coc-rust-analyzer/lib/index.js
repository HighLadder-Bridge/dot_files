  ...
  "node_modules/vscode-jsonrpc/lib/common/linkedMap.js"(exports2) {
    ...
    var LinkedMap = class {
      ...
      before(key) {
        ...
        return item ? (_a = item.previous) == null ? void 0 : _a.value : void 0;
      }
      after(key) {
        var _a;
        const item = this._map.get(key);
        return item ? (_a = item.next) == null ? void 0 : _a.value : void 0;
      }
      has(key) {
...
var require_connection = __commonJS({
  "node_modules/vscode-jsonrpc/lib/common/connection.js"(exports2) {
    ...
    function createMessageConnection(messageReader, messageWriter, _logger, options) {
      ...
      const version = "2.0";
      const maxParallelism = (options == null ? void 0 : options.maxParallelism) ?? -1;
      let inFlight = 0;
        ...
        onNotification: (type, handler) => {
          ...
          return {
            dispose: () => {
              var _a;
              if (method !== void 0) {
                if (((_a = notificationHandlers.get(method)) == null ? void 0 : _a.handler) === handler) {
        ...
        onRequest: (type, handler) => {
          ...
          return {
            dispose: () => {
              ...
              if (method !== void 0) {
                if (((_a = requestHandlers.get(method)) == null ? void 0 : _a.handler) === handler) {
...
var require_protocol = __commonJS({
  "node_modules/vscode-languageserver-protocol/lib/common/protocol.js"(exports2) {
    ...
    (function(CodeActionRequest3) {
      ...
      CodeActionRequest3.capabilities = messages_1.CM.create("textDocument.codeAction", "codeActionProvider");
    ...
    (function(CodeActionResolveRequest3) {
      ...
      CodeActionResolveRequest3.capabilities = messages_1.CM.create("textDocument.codeAction.resolveSupport", "codeActionProvider.resolveProvider");
...
var import_vscode_languageserver_protocol2 = __toESM(require_api2());
...
var import_vscode_languageserver_protocol = __toESM(require_api2());
...
function isCargoTomlDocument(document) {
  const u = import_coc4.Uri.parse(document.uri);
  ...
  resolveBin() {
    ...
    if (systemBin) {
      ...
      if (stderr.trim().length > 0) {
        ...
        import_coc4.window.showWarningMessage(errMsg);
        return;
      }
...
function matchingBrace2(ctx) {
  return async () => {
    ...
    if (response.length > 0) {
      import_coc5.workspace.jumpTo(document.uri, response[0]);
    }
  };
}
...
function ssr2(ctx) {
  return async () => {
    ...
    const mode = await import_coc5.workspace.nvim.call("visualmode");
    if (mode) {
      const range = await import_coc5.window.getSelectedRange(mode.toString());
...
function viewSyntaxTree2(ctx) {
  return async () => {
    ...
    const mode = await import_coc5.workspace.nvim.call("visualmode");
    let range = null;
    if (mode) range = await import_coc5.window.getSelectedRange(mode.toString());
    const param = {
      textDocument: { uri: doc.uri },
      range
    };
...
function viewMemoryLayout(ctx) {
  return async () => {
    ...
    const res = await ctx.client.sendRequest(viewRecursiveMemoryLayout, param);
    if (!res || res.nodes.length === 0) return;
    const lines = [];
    const process2 = (idx, depth) => {
    ...
    const nvim2 = import_coc5.workspace.nvim;
    nvim2.pauseNotification();
    nvim2.command("edit +setl\\ buftype=nofile [Layout]", true);
    nvim2.command("setl nobuflisted bufhidden=wipe", true);
    nvim2.call("append", [0, lines], true);
    nvim2.command("exe 1", true);
    await nvim2.resumeNotification(true);
...
function explainError(ctx) {
  return async () => {
    ...
    const { document, position } = await import_coc5.workspace.getCurrentState();
    if (!isRustDocument(document)) return;
    const diag = (_b = (_a = ctx.client.diagnostics) == null ? void 0 : _a.get(document.uri)) == null ? void 0 : _b.find((diagnostic) => isInRange(diagnostic.range, position));
...
async function applySnippetWorkspaceEdit(edit) {
  ...
  for (const change of edit.documentChanges) {
    if ("kind" in change || !import_vscode_languageserver_protocol2.TextDocumentEdit.is(change)) {
      continue;
    }
    const newEdits = [];
    ...
    await import_coc5.workspace.applyEdit({
...
function runFlycheck2(ctx) {
  return async () => {
    const { document } = await import_coc5.workspace.getCurrentState();
    if (!isRustDocument(document)) return;
    ctx.client.sendNotification(runFlycheck, { textDocument: { uri: document.uri } });
  };
}
...
function viewFileText2(ctx) {
  return async () => {
    const { document } = await import_coc5.workspace.getCurrentState();
    if (!isRustDocument(document)) return;
    const ret = await ctx.client.sendRequest(viewFileText, { uri: document.uri });
    if (!ret) return;
    const nvim2 = import_coc5.workspace.nvim;
    nvim2.pauseNotification();
...
function peekTests(ctx) {
  return async () => {
    ...
    await import_coc5.commands.executeCommand("editor.action.showReferences", import_coc5.Uri.parse(document.uri), position, locations);
  };
}
...
async function activate(context) {
  ...
  ctx.registerCommand("viewMemoryLayout", viewMemoryLayout);
